...
from toolkit.utils import Ontology, propagate_annots
from toolkit.models import GINModel
from toolkit.data import load_ppi_data
from toolkit.metrics import compute_roc
...
def main(data_root, ont, model_name, test_data_name, batch_size, epochs, load, device):
    ...
    mfs_dict, terms_dict, graph, train_nids, valid_nids, test_nids, data, labels, test_df, go_embedding, nids = load_ppi_data(
    ...
    net = GINModel(features_length, n_terms, device).to(device)
    print(net)
    ...
    if not load:
        ...
        for epoch in range(epochs):
            ...
            with tqdm(total=train_steps) as bar:
                for input_nodes, output_nodes, blocks in train_dataloader:
                    bar.update(1)
                    logits = net(input_nodes, output_nodes, blocks, graph, go_embedding)
            ...
            with th.no_grad():
                ...
                valid_loss = 0
                preds = []
                with tqdm(total=valid_steps) as bar:
    ...
    with th.no_grad():
        ...
        test_loss = 0
        preds = []
        with tqdm(total=test_steps) as bar:
