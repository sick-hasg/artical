...
def sum_pooling(x, n_num):
    group_size = x.shape[0] // n_num
    truncate_size = group_size * n_num
    x_truncated = x[:truncate_size]
    x_reshaped = x_truncated.view(n_num, group_size, 1024)
    x_pooled = th.sum(x_reshaped, dim=1)
